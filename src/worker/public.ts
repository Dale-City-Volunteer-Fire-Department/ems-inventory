...
import { getMagicLinkToken } from './auth/magic-link';
...
/**
 * Validate a public token (PIN-generated) and return its data, or null if invalid.
 */
...
/**
 * Validate either a PIN public token OR a magic link token.
 * Magic link tokens use the `magic:` KV prefix and are looked up via getMagicLinkToken.
 * Returns { tokenData, submitterEmail } where submitterEmail is null for PIN tokens.
 */
async function validateAnyPublicToken(
  ...
  // Try magic link first (longer hex strings generated by HMAC)
  const magicData = await getMagicLinkToken(kv, token);
  if (magicData) {
    // Magic link tokens don't have per-token submission counters — use synthetic data
    const syntheticTokenData: PublicTokenData = { created: magicData.created_at, submissions: 0, uploads: 0 };
    return { tokenData: syntheticTokenData, submitterEmail: magicData.email };
  }

  // Fall back to PIN public token
  const pinData = await validatePublicToken(kv, token);
  if (pinData) {
    return { tokenData: pinData, submitterEmail: null };
  }
...
export async function handlePublicUpload(request: Request, env: Env): Promise<Response> {
  try {
    ...
    // H-2: Check upload cap (PIN tokens only — magic link tokens use rate limiting instead)
    if (submitterEmail === null && tokenData.uploads >= MAX_UPLOADS_PER_TOKEN) {
    ...
    // H-2: Increment upload counter (PIN tokens only)
...
export async function handlePublicInventorySubmit(request: Request, env: Env): Promise<Response> {
  try {
    ...
    // H-2: Check submission cap (PIN tokens only — magic link tokens use rate limiting)
    if (submitterEmail === null && tokenData.submissions >= MAX_SUBMISSIONS_PER_TOKEN) {
    ...
    // H-2: Increment submission counter (PIN tokens only)
    if (submitterEmail === null) {
